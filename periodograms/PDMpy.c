...
static double delf, df, dt_avg, dt_min, segmn[MAXSEGS+1], seg_var[MAXSEGS+1], seg_var0[MAXSEGS+1];
...
double segdev = 100;						 /*  sensitivity for segments (big->1 seg)  */
...
int super_plot = 0;						/*  write unneeded files				  */
...
int pdm_verbose = FALSE;					   /*  generate screen output				 */
int pdm_debug = FALSE;						 /*  turn on local debugging				*/
...
int main(int argc, char *argv[]) {
    if (argc != 4) {
        printf("Usage: %s minf0 maxf0 df\n", argv[0]);
        return 1;
    }

    double minf0 = atof(argv[1]);
    double maxf0 = atof(argv[2]);
    double df = atof(argv[3]);

    // The arrays will be passed from Python as function arguments
    // In this example, I'll assume you pass the arrays as command-line arguments
    int Numdat = argc - 4;
    double *times = (double *)malloc(Numdat * sizeof(double));
    double *mags = (double *)malloc(Numdat * sizeof(double));
    double *sigs = (double *)malloc(Numdat * sizeof(double));

    for (int i = 0; i < Numdat; i++) {
        times[i] = atof(argv[i + 4]);
        mags[i] = atof(argv[i + 4 + Numdat]);
        sigs[i] = atof(argv[i + 4 + 2 * Numdat]);
    }

    // Call the pdm_analysis function with the provided arguments
    pdm_analysis(minf0, maxf0, df, times, mags, sigs, Numdat);
...

{
	...
	/*--------------------------------------------------------*/
		...
		for( f = f_min, k = 0; f <= f_max + delf; f += delf ) {
			...
			fprintf( fp, "%g,%g\n", f, theta );			/* THIS IS WHERE IT IS WRITTEN TO THE FILE */
		...
		//NM: We dont need this shit so i removed it with the 'super_plot' boolean
