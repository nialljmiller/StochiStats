...
def source_lc_gen(mag, magerr, time, N, median_mag, time_range):
    ...
    def seed_gen(N, median_mag):
        ...
        if median_mag is None:
            median_mag = np.random.uniform(11, 16)
...
def synthesize(mag = None, magerr = None, time = None, N = None, period = None, amplitude = None, median_mag = None, other_pert = 1, scatter_flag = 1, contamination_flag = 1, err_rescale = 1, cat_type = None, time_range = [0.5,3000]):
    ...
    seed_m = np.random.uniform(mag25, mag75, n_pts)

    magerr25, magerr75 = np.percentile(magerr, [25,75])
    seed_em = np.random.uniform(magerr25, magerr75, n_pts)


    cat_types = ['Ceph', 'EB', 'RR', 'YSO', 'CV']

    if cat_type == None or cat_type not in cat_types:
        cat_type = np.random.choice(cat_types)  # If no type, np.randomly pick any

    if isinstance(cat_type, str) == False:
        cat_type = np.random.choice(cat_type)  # If cat type has multiple types, np.randomly pick one
    ...
    if period == None:
        ...
        if cat_type == 'RR':
            period = np.random.uniform(0.1, 3)
        if cat_type == 'YSO':
            period = np.random.uniform(0.1, 14)
        if cat_type == 'CV':
            period = np.random.uniform(3, 30)

    if amplitude == None:
        # If no amplitude, pick one that's above the noise
        avg_error = np.median(seed_em)
        amplitude = np.random.uniform(3 * avg_error, 10 * avg_error)


    sim_mags, cat_type = lc_model(seed_t, period, amplitude, cat_type)
    ...
    if scatter_flag == 1:
        try:
            old_mags = seed_m
            seed_m = [np.random.normal(float(mag), float(seed_em[i])) for i, mag in enumerate(old_mags)]
